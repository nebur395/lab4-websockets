...
public class ElizaServerTest {
	...
	public void onChat() throws DeploymentException, IOException, URISyntaxException, InterruptedException {
		...
		ClientEndpointConfig configuration = ClientEndpointConfig.Builder.create().build();
		ClientManager client = ClientManager.createClient();
		client.connectToServer(new ElizaEndpointToComplete(list, latch), configuration, new URI
                ("ws://localhost:8025/websockets/eliza"));
		latch.await();
    ...
    private static class ElizaEndpointToComplete extends Endpoint {
        ...
        public void onOpen(Session session, EndpointConfig config) {
            ...
            session.getAsyncRemote().sendText("You are my best friend.");
